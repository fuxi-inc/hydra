...
func createPlan(t *testing.T, extra url.Values, isParallel bool) {
	...
	//require.NoError(t, err)
	body := makePost(t, urlx.CopyWithQuery(urlx.AppendPaths(server, "/identifier/plan"), extra).String(),
	...
	gjson.GetBytes(body, "modules").ForEach(func(_, v gjson.Result) bool {
		...
		t.Run("testModule="+module, func(t *testing.T) {
			...
			for retry := 1; retry <= maxRetries; retry++ {
				...
				body := makePost(t, urlx.CopyWithQuery(urlx.AppendPaths(server, "/identifier/runner"), params).String(),
					nil, 201)
				...
				for {
					...
					case "oidcc-server-rotate-keys":
						if state == "CONFIGURED" {
							...
							makePost(t, urlx.AppendPaths(server, "/identifier/runner/", gjson.GetBytes(body, "id").String()).String(), nil, 200)
						}
					}
				}
			}
...
func checkStatus(t *testing.T, testID string) (string, status) {
	res, err := httpClient.Get(urlx.AppendPaths(server, "/identifier/info", testID).String())
	require.NoError(t, err)
	defer res.Body.Close()
	body, err := ioutil.ReadAll(res.Body)
	require.NoError(t, err)
