...
func (h *Handler) SetRoutes(public *x.RouterPublic) {
	...
	public.GET(IdentifierHandlerPath+"/:id"+AddressHandlerPath, h.Get)
}
...
func (h *Handler) GetIdentifierAddr(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	var id = ps.ByName("id")

	ctx := context.Background()

	entity, err := h.r.IdentifierManager().GetIdentifierAddr(ctx, id)
	if err != nil {
		h.r.Writer().WriteError(w, r, err)
		return
	}
